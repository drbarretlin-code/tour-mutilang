...
### OSM Nominatim API 錯誤處理
- **防呆與限流提示**：在 `AttractionAutocomplete.tsx` 中新增了針對 HTTP 429 (Too Many Requests) 的例外處理。若使用者打字過快或請求過於頻繁觸發了官方的限流機制（每秒 1 次），系統將不再默默失效，而是會在下拉選單中明確提示「搜尋請求過於頻繁（每秒最多 1 次），請稍候 3 秒後再試」，引導使用者恢復正常操作。
