...
// 5. 替換活動卡片的連結渲染 UI
// 為了避免因空格換行造成不匹配，我們先抓取 links 渲染區塊的前後關鍵字
...
// 我們可以在 `const [activeTab, setActiveTab] = useState('overview');` 之後插入
...
// 由於原本的寫死區塊很長，我們定位其前後特徵：`{/* 互動點擊區域 */}` 到 `)}`
// 尋找：
//                     {/* 互動點擊區域 */}
//                     {!generateError && (
//                       <>
...
// 找到 startMarker 後，往後找第一個 {!generateError && (
...
// 我們可以用 `</div>\n                    )}` 或是類似定位來切
const endMarker = '</div>\n                    )}';
let endIndex = content.indexOf(endMarker, errorCheckIndex);
if (endIndex === -1) {
  // 嘗試另一種排版
  const endMarkerAlt = '</div>\n                  )}';
  endIndex = content.indexOf(endMarkerAlt, errorCheckIndex);
  if (endIndex === -1) {
    console.error('找不到 互動點擊區域 的終點！');
    process.exit(1);
  }
}

// 將該段落整塊替換
const overlayOld = content.substring(startIndex, endIndex + endMarker.length);
